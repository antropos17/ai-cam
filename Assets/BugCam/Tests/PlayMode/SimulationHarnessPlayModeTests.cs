...
{
    /// <summary>
    /// Local PhysicsScene creation via SceneManager.CreateScene(LocalPhysicsMode.Physics3D)
    /// is Play Mode-only. These tests live in the PlayMode assembly instead of using
    /// EnterPlayMode workarounds from EditMode.
    /// </summary>
    public sealed class SimulationHarnessPlayModeTests
    {
        ...
        public IEnumerator RunSimulatesOneBodyInFreshLocalPhysicsScene()
        {
            ...
            Assert.That(
                ...
                "SimulationHarness.Run must unload its fresh local physics scene within 10 play-mode frames.");
        }
