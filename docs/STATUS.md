...
## Current position
- Active block: **2.2.1 — Polish pass** (user-inserted, branch `feat/block-2.2.1`, commits `block-2.2.1:`, evidence `Library/BugCamEvidence/Block2.2.1-*`). Composition (buckets A/B/C), number 2.2.1, and the A1/A2/A3 contract ratified by the human 2026-08-03. Ratified open decisions: SphereCollider supported in capture (`Size` = diameter); kinematic-with-Animator ⇒ freeze to static + warning propagated to window AND result verdict AND manifest/evidence; A3 fallback (removing the score half if measurement shows it never decides) is NOT automatic — STOP and adjudicate over the measured data. Work order: A6+A7 → A4 → A1 → A3 → A2 (sub-stage) → A5 → A8 (exit gate: dominoes scene). Progress: **A6+A7+A4 DONE** (Evidence log 2026-08-03 Block 2.2.1 below; batchmode 107/107 + 21/21); next: A1 per the ratified contract. See PLAN.md Block 2.2.1. Full ratified contract text (incl. the A1 validation table with verbatim reason strings): `docs/CONTRACT-2.2.1.md` — written 2026-08-03 because the full text previously existed only in chat (single point of failure).
