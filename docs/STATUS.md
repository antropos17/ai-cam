...
## Current position
- Active block: 2.2-window-ux (Ghost Visualization window UX pass on `feat/block-2.2-window-ux`, user-inserted block — not PLAN's "Block 2.2 Evidence overlay + export", which remains not started). Code, review fixes, live two-run verification, MCP screenshots of every window state, and regression pins are DONE (see Evidence log); awaiting the final batchmode `-Suite All` gate on the branch tip and merge of PR #8.
