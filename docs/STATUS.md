...
## Current position
- Active block: **2.2.1 — Polish pass** (user-inserted, branch `feat/block-2.2.1`, commits `block-2.2.1:`, evidence `Library/BugCamEvidence/Block2.2.1-*`). Composition (buckets A/B/C), number 2.2.1, and the A1/A2/A3 contract ratified by the human 2026-08-03. Ratified open decisions: SphereCollider supported in capture (`Size` = diameter); kinematic-with-Animator ⇒ freeze to static + warning propagated to window AND result verdict AND manifest/evidence; A3 fallback (removing the score half if measurement shows it never decides) is NOT automatic — STOP and adjudicate over the measured data. Work order: A6+A7 → A4 → A1 → A3 → A2 (sub-stage) → A5 → A8 (exit gate: dominoes scene). Progress: **A6+A7+A4+A1 DONE** — A1 reviewed (fix `c437e8a` accepted) and squash-merged as PR #9, squash SHA `7bbaa45c17de5e24be08227a34cc0af89cf1b29b`; merge performed with head-SHA match (local == origin == PR headRefOid), local `main` fast-forwarded, `feat/block-2.2.1` reset onto the squash (tree-identical, verified). **A3 DONE and CLOSED** (measurement → adjudicated threshold 0.2 → max formula applied → double control bit-identical → score-half adjudicated 2026-08-03: **KEPT**, with a binding A8 re-measurement obligation; see the A3 Evidence entries). **A2 DONE — review passed 2026-08-03** (fail-closed decisions ratified as recorded; adjudicated follow-up landed: sleeping-body capture notice to report+manifest, verdict untouched — see the follow-up Evidence entry). **A5 DONE** (bit-identical identical-config search-repeat pinned by test AND by a host-path repeat with an empty field-by-field diff — closes PLAN 1.4 VERIFY (a); see the A5 Evidence entry). Still pending from A2: live window screenshots of scene-mode states (focused-editor session, do not block). **A8 DONE 2026-08-03** — exit gate passed end-to-end through the A2 captured-scene path on the new committed `Assets/BugCam/Tests/DominoScene.unity` (verdict THRESHOLD BRACKET FOUND); the binding A3 re-measurement executed on domino data (480 steps: agreement 202/480, 103 score-fires/pos-holds, **175 opposite-direction**) and the **score-half adjudication is CLOSED PERMANENTLY** (ruling 2026-08-03: score half KEPT as load-bearing; deletion question closed without right of reopening on this contract); tower gate re-verified bit-identical to the A3 pin; batchmode EditMode 134/134 + PlayMode 24/24 — see the A8 Evidence entry. Block 2.2.1 work items are complete; block-closure items (CS0162 pragma decision as a separate documented commit, stepCount default review, merge after review) are a separate step after human review. See PLAN.md Block 2.2.1. Full ratified contract text (incl. the A1 validation table with verbatim reason strings): `docs/CONTRACT-2.2.1.md` — written 2026-08-03 because the full text previously existed only in chat (single point of failure).
...
| 2.2.1 | Polish pass: A1 search-entry parameterization; A3 max-norm score + threshold 0.2 (score-half adjudication closed permanently at A8); A2 arbitrary-scene capture + sleeping-body notice; A4 Scene View legend; A5 bit-identical repeat pin (closes PLAN 1.4 VERIFY (a)); A6+A7 docs; A8 domino exit gate through the capture path + agreement re-measurement | See Evidence log 2026-08-03 (A1…A8 entries); final gate EditMode 134/134 + PlayMode 24/24 (`Block2.2.1-a8`) | A1 squash-merged as PR #9 `7bbaa45`; A2–A8 on `feat/block-2.2.1` — merge after review; block closure (CS0162 pragma, stepCount default review) pending as a separate step |

## Open findings / blockers
- OPEN (product, recorded 2026-08-03 during the human's first real A1 window session — record only, nothing decided): **with the default 32 steps the max spread is 3.79 mm — physically invisible in Scene View at any reasonable zoom**, so the ghost fan reads as "nothing happened" despite honest numbers. The hero demo and any visual evidence need a longer sim window. Reconsider the default step count (or make the window hint that visible fans need N steps). Related instrumentation datum: the 32-step window is also analytically tight — first divergence frame 27 + 5 sustained steps = exactly 32 (A3 step-1 Evidence entry, finding 3). Adjudication 2026-08-03: default step count NOT changed now; verified fact — every pinned regression test already passes its step count explicitly (`VerifyStepCount=32`, `FastStepCount=40`, checkpoint 250 — constants in test sources, none relies on a default), so the default can be reconsidered at block close without breaking pins. Third confirmation 2026-08-03 (Block 2.2.1 A8, adjudicated record): the domino chain physically cannot develop within 32 steps (0.64 s for a 5-domino run) — chain bodies stayed ≤ 0.34 mm apart between baseline and perturbed runs while the verdict bracket formed on the target body alone; scene deliberately NOT tuned (the gate proves the pipeline, spectacle belongs to the hero demo, Block 2.5); see the A8 Evidence entry.
