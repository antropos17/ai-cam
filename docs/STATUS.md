...
## Current position
- **CLOSURE 2026-08-05 — Block 2.2.3 contract AMENDMENTS (five, 2026-08-05) MERGED to `main` as PR #14, squash SHA `a0fa5973e102349b59a8d8e5e7c1bf035ba6ff04` = new `main` tip; merge guarded with `--match-head-commit 8af3feeee23559b49fa05a623e03c2dfa5e9196c`** (local == origin == PR headRefOid at merge time; merge-base `7cf34f6` = the pre-merge `main` tip, as claimed by the 2026-08-05 entry below; local `main` fast-forwarded `7cf34f6..a0fa597`; the two branch commits `03e417a` and `8af3fee` were neither rebased nor amended and no force push was made). Three files landed and nothing else: `docs/CONTRACT-2.2.3.md` (638 → 928 lines, +290, blob `636033335d1d555d6eabbcbea1eb55592895aedc` on `main` — byte-identical to the branch version, verified by blob hash after the merge; the ratified text itself was not re-opened, superseded wording is left visible per the 2.2.2 amendment convention), `docs/PLAN.md` (one line — the Block 2.2.3 entry now points at the amendments) and `docs/STATUS.md` (Active-block line marked AMENDED plus the 2026-08-05 amendment entry). **The split provenance of the amendment text survives the merge in its three classes and must not be flattened:** the bodies of amendments 1, 2, 5 and the rows 9/12 clarification came from the human intact; the three openings (amendments 3, 4 and the RULES first line) were reconstructed by the agent and **confirmed by the human 2026-08-05** together with the AND conjunction in amendment 3 and the `SNAPSHOT_CAPTURE_FAILED` binding incl. its «Код ошибки» footnote, each confirmed as a separate ruling; all other supporting text was **drafted by the agent** and is attributed item by item in the contract's «Провенанс» → «Авторство вспомогательного текста (2026-08-05)». The amendments themselves are not restated here — the 2026-08-05 entry below carries them, and its «PR open, NOT merged» clause is **superseded on the merge-status point only** by this record and is deliberately left as written. **Block 2.2.3 code has NOT begun — amending the contract is not a start of implementation: no C# written or modified, no Unity session, no batchmode run, no test suite executed, no scene opened, no pin exercised.** Branch `feat/block-2.2.3-impl` is deleted locally and on the remote immediately after this record; HEAD stays on `main` and no successor branch is created in this session.
- **CLOSURE 2026-08-04 — Block 2.2.3 ratified contract MERGED to `main` as PR #13, squash SHA `6d7e27c64a0139a3581494b4db7241650984bbe6` = new `main` tip; merge guarded with `--match-head-commit 59f8e091ad80026c0a50e66befd6d62dc59b5777`** (local == origin == PR headRefOid at merge time; local `main` fast-forwarded `4ea3756..6d7e27c`). Three files landed and nothing else: `docs/CONTRACT-2.2.3.md` (new, 638 lines, blob `154bb3439b9c1785921c2cfedb51863686765dc3` — the ratified text, byte-identical before and after the PR, not re-opened), `docs/PLAN.md` (Block 2.2.3 heading, precedence pointer to the contract, P.6 question marked RESOLVED by adjudication decision №1 with the original wording left visible) and `docs/STATUS.md` (Active-block line, reframe Evidence-log Environment note). **The ratified contract for Block 2.2.3 is present on `main` and Block 2.2.3 code has NOT begun — ratification is not a start of implementation: no C# written or modified, no Unity session, no batchmode run, no test suite executed, no scene opened, no pin exercised and none can have drifted.** **Audit findings 5 and 6 are resolved by the contract file now existing on `main`** — `docs/PLAN.md` Block 3.0 probe 1 and the precedence block of `docs/RESEARCH-2026-08-04-placement.md` both cite `docs/CONTRACT-2.2.3.md`, and that citation resolves on `main` for the first time; the three BLOCKING bookkeeping findings (the `contract not written` collision against the contract's own STOP-on-contradiction header rule) are closed by the same merge. Branch `feat/block-2.2.3` is deleted locally and on the remote immediately after this record; HEAD stays on `main` and no successor branch is created in this session.
- **CLOSURE 2026-08-04 — Product reframe MERGED to `main` as PR #12, squash SHA `b6d36ffbc897bb1233ec8ff5f67deee8abc69c4a` = new `main` tip; merge guarded with `--match-head-commit 52779a69a342b3293c0a44a1c30b510fa54e227c`** (head unchanged between approval and merge; local `main` fast-forwarded). Seven files touched: `docs/RESEARCH-2026-08-04-placement.md` (new), `docs/SPEC.md` (§20, §21, §14, §15), `docs/PLAN.md` (Phase 3 + the DAY 3 UPM-README caveat amendment), `docs/STATUS.md`, `CLAUDE.md`, `AGENTS.md`, `README.md`. **The reframe is documentation only — zero measurements taken, no C# written or modified, no Unity run, no test suite executed, no pin exercised — and Phase 3 remains `PLANNED-NOT-RATIFIED`, blocked on the two empirical probes in `PLAN.md` Block 3.0.** Branch `docs/product-reframe` deleted locally and on the remote after the merge; `feat/block-2.2.3` deliberately untouched.
- **Product reframe recorded 2026-08-04 (docs-only, branch `docs/product-reframe`; changes nothing about the active block).** BugCam's measurement is one comparison run against four reference points (`SPEC.md` §20), and the placement detector family (`SPEC.md` §21) is added as an explicit **scope expansion that is NOT part of v0.1**, planned in `PLAN.md` **Phase 3** with every block marked `PLANNED-NOT-RATIFIED` and blocked on contract ratification. **Blocks 2.2.3 → 2.6 are unaffected and remain the critical path** — the evidence layer they build is required under all four reference points, placement findings included. Two probes block the Phase 3 contract (`PLAN.md` Block 3.0); two hard constraints are recorded under Open findings. Research provenance, not re-verified in this repository: `docs/RESEARCH-2026-08-04-placement.md`. Full entry: Evidence log 2026-08-04, product reframe.
- **2026-08-05 — Block 2.2.3 contract AMENDED: five amendments, docs only, branch `feat/block-2.2.3-impl` (cut from `main` `7cf34f6`), PR open, NOT merged.** [Superseded on the merge-status point by the CLOSURE 2026-08-05 record above — PR #14 squash-merged as `a0fa5973e102349b59a8d8e5e7c1bf035ba6ff04`, branch deleted.] **Provenance is split and must not be flattened:** the bodies of amendments 1, 2, 5 and the rows 9/12 clarification came from the human intact; the three openings (amendments 3, 4 and the RULES first line) were reconstructed by the agent and **confirmed by the human 2026-08-05** together with the AND conjunction in amendment 3 and the `SNAPSHOT_CAPTURE_FAILED` binding incl. its «Код ошибки» footnote; all other supporting text (code citations, line coordinates, rationale, footnotes) was **drafted by the agent** and is attributed item by item in the contract's «Провенанс» → «Авторство вспомогательного текста (2026-08-05)». Landed in `docs/CONTRACT-2.2.3.md` (new section «Поправки 2026-08-05» plus in-place pointers; superseded wording left visible per the 2.2.2 amendment convention) with consistency-only touches to `docs/PLAN.md` and this file: (1) `Assets/BugCam/Editor/SceneCaptureHandover.cs` enters the surface of change — `BodyDto` must carry `angularVelocity`, `startsAsleep`, `maxAngularVelocity`, because without them the injected state is lost across domain reload and a snapshot run **silently degrades into an ordinary scene run** (silent degradation, not failure, is why this is mandatory); (2) matching lives inside `SceneCapture.Capture` per decision Р1(a) and the `#NN`/`#rb` key scheme is never exposed on `SceneCaptureResult` or any public surface — the earlier plan wording about exposing static keys outward is **withdrawn**; (3) the structured launch entry point is non-public and differently named because a second public static `TryStartTowerSearch` overload would throw `AmbiguousMatchException` at four existing reflection call sites (`GhostEvidenceTests.cs:547-551/642-645/692-695`, `SearchEntryParameterizationTests.cs:377-378`) — the existing public signature stays byte-for-byte and delegates; (4) bodies and statics are compared as separate sets (`ExcludedSafely` excluded, same membership criterion as an ordinary capture minus mesh resolution); (5) the row-12 scale channel is `StaticCandidate.SourceLossyScale` inside `SceneCapture.Capture`, compared before `SceneCaptureResult` construction and before handover serialization, with `SimulationStaticColliderDefinition.FullScale` left mesh-only — permitted by the pin re-run `Library/BugCamEvidence/probe-2.2.3-v1-pins/REPORT.md` (pins 1–3 bit-identical, field-diff zero on tower and domino, EditMode 157 / PlayMode 27 unchanged, handover round-trip pin green, 1-ULP injected drift flagging all four `GateScene222` statics by real key while a clean pass flagged none; caveat raised by the measuring agent: pin 1 never enters `SceneCapture.Capture`, so pins 2 and 3 are the discriminating ones; the probe code was reconstructed from constraints and never committed). Rows 9 and 12 print the full `Vector3` in `"R"`/`InvariantCulture` — `objectScale` must not be used. **No implementation code was written in this session: no C# touched, no Unity session, no batchmode run, no test suite executed, no pin exercised.**
- Active block: **Block 2.2.3 — Play Mode snapshot capture — CONTRACT RATIFIED 2026-08-04, AMENDED 2026-08-05 (five amendments), CODE NOT STARTED** (position set 2026-08-04 immediately after the PR #11 squash-merge; contract written and adjudicated on branch `feat/block-2.2.3`, no implementation work begun). Resolved 2026-08-04: the earlier PLAN-vs-STATUS discrepancy was PLAN staleness, not a typo — Block 2.2.3 was decided in the 2026-08-04 planning session as an explicit insert between 2.2.2 and 2.3 and is now written into `docs/PLAN.md` (Block 2.2.3 entry; blocks 2.3–2.6 unchanged and unrenumbered). Per the 2.2.1/2.2.2 precedent, `docs/CONTRACT-2.2.3.md` was adjudicated before any code: full ratified text on the branch, open questions №1–№11 plus the literal-11 fork closed, empirical preconditions П-1…П-5 executed and green. Ratification is not a start of code — implementation of the block has not begun.
- Block 2.2.2 — Capture universality: **MERGED to `main` 2026-08-04 as PR #11, squash SHA `855911ccb458f4ff2c690813b64dd95e3bb2a3a0` = new `main` tip; merge guarded with `--match-head-commit 4713bdb775c83984dc9ed2979ce92c8f1ec805eb`** (head unchanged since the block-close check). (user-inserted 2026-08-03 ahead of Block 2.3; branch `feat/block-2.2.2`). Contract written 2026-08-03 «contract first, no code», adversarially verified (26 checks, three fixes), open questions №1–№10 adjudicated by the human 2026-08-04, **ratified 2026-08-04**: full text `docs/CONTRACT-2.2.2.md` (PLAN Block 2.2.2 is the summary; contradiction = STOP). The contract's mandatory empirical precondition **executed 2026-08-04** (see Evidence log): edit mode reads Read/Write-disabled mesh geometry, Play Mode does NOT (editor `AssetDatabase` channel included), structural fingerprint (vertexCount / subMeshCount / bounds) and MeshCollider cooking DO work in Play Mode ⇒ **Amendment 2026-08-04 ratified by the human**: full contentHash moves to an edit-mode capture point before Play Mode entry; the simulation point verifies a structural fingerprint; `SCENE_MESH_RESOLVE_FAILED` fires on unresolved mesh or fingerprint mismatch. Amendment wordings approved 2026-08-04; **implementation LANDED** (`dc64ac2`/`d72f88b`/`f8bb4ca`/`7be8601`); **exit gate COMPLETE 2026-08-04**: assembled third-party mesh scene (KayKit Dungeon Remastered 1.0, CC0, https://github.com/KayKit-Game-Assets/KayKit-Dungeon-Remastered-1.0 — ruling 7 amendment) captured with zero fail-closed objects and searched end-to-end; verdict **DIVERGENT AT SEARCH FLOOR accepted by the human as the final gate result** (intentionally unstable scene, honest verdict, no tuning per the A8 precedent); tower and domino pins bit-identical after every change (see the exit-gate Evidence-log entry). **Block 2.2.2 work COMPLETE on the branch; squash-merged to `main` 2026-08-04 as PR #11 (see the merge record above).** After 2.2.2 per PLAN order: **Block 2.2.3 — Play Mode snapshot capture** (inserted into PLAN 2026-08-04; Block 2.3 — Evidence overlay + export follows it). Carried over: live window screenshots of scene-mode states (PENDING since 2.2.1, does not block).
...
- RULE (standing, recorded 2026-08-05): `Library/` stays gitignored because Unity regenerates it, therefore any measurement that must survive a clean checkout is transcribed into `docs/` — this applies to the Block 2.2.3 exit-gate artifacts as well.
- KNOWN ISSUE (→ Phase 3 Block 3.0 probe 2, recorded 2026-08-04 from the research session, NOT re-verified in this repository): **raw `Physics.ComputePenetration` output cannot be trusted without sanitization.** Documented defect class — a capsule against a non-convex mesh returns an absurd depth of **743.8444** for a shallow overlap, or returns `false` at small depth; the community workaround is to call the function **16 or more times** at slightly varied positions. **Cost implication: the flagship static clash detector may be up to 16× more expensive than assumed**, which is a design input, not a constant. Blocking probe: `docs/PLAN.md` Block 3.0 probe 2 measures reproduction frequency on convex versus non-convex pairs and the real wall-clock cost of the multi-sample workaround on this project's Unity version. Now also a caveat in `docs/SPEC.md` §14. Source and standing: `docs/RESEARCH-2026-08-04-placement.md`.
- KNOWN ISSUE (product-wide claim boundary, recorded 2026-08-04 from the research session): **PhysX determinism is a within-platform and within-version claim only.** It holds for identical hardware and identical PhysX version; results **diverge across CPU vendors** (Intel versus AMD) and due to **GPU operation ordering**. Such divergence is **expected, not a defect**, and must never be reported as one. `docs/SPEC.md` §14 now states this directly, replacing the softer "repeatability is environment-scoped" wording; `SPEC.md` §13 already excluded absolute cross-platform repeatability from v0.1, so this tightens the wording rather than changing scope. No existing measurement or pin is invalidated — every pin in this repository is a within-platform result recorded on one machine. Source and standing: `docs/RESEARCH-2026-08-04-placement.md`.
- OPEN (backlog, recorded 2026-08-04, no code implied, **resolve at phase close**): **the existing amplification metric must be reconciled with the finite-time Lyapunov exponent definition and relabelled accordingly.** Precise statement of the gap, so the work is well-posed rather than a rename: amplification today is a **ratio**, `maxSpread(m) / epsilon(m)` (`PLAN.md` Block 1.3; sold as such in `SPEC.md` §2 and §10; live value `4535758×` in the 250-step entry below), whereas the finite-time Lyapunov exponent is `Λ = (1/T)·ln(‖final perturbation‖ / ‖initial perturbation‖)` — **the natural log of that ratio divided by the window length**, a different quantity with different units, not a renamed one. The literature is explicit that finite-time exponents fluctuate, depend on the reference trajectory, and do not converge to the asymptotic Lyapunov exponent over a short window; consequently, if the metric is ever presented in Lyapunov terms it **must always be labelled finite-time** and never called "the Lyapunov exponent" without that qualifier. **Nothing changed now:** no number, no formula, no `SPEC.md` §2 / §10 / §17 text, no overlay wording. Source and standing: `docs/RESEARCH-2026-08-04-placement.md`, Mathematical foundations.
- CONSTRAINT (→ Block 2.3, recorded 2026-08-04 before the block starts): `Tools/BugCam/run-checkpoint.ps1` launches Unity with `-nographics`, so no graphics device is initialised and **video cannot be an artifact of the current checkpoint harness under any encoder**. Numbers are certified by the harness; video is not. Video verification requires a separate GPU-enabled path, to be defined in the Block 2.3 contract.
- OPEN (backlog, recorded 2026-08-04, no code implied): MediaEncoder defaults to H.264 Baseline profile level 4.2 — accepted as-is for v0.1; if output quality proves insufficient for the landing page or hero video, revisit with `VideoTrackEncoderAttributes` + `H264EncoderAttributes` at Main or High profile. Companion limitation: the no-sRGB-shift measurement holds for a Gamma color space project only — a Linear project requires a separate measurement before the same claim is made (see Evidence log 2026-08-04, MediaEncoder probe).
- OPEN (cosmetic, backlog, carried from Block 2.2.2): tower control runs record `scenePath=Assets/BugCam/Tests/DominoScene.unity` in metrics while the tower itself is procedural (the open editor scene is irrelevant to the tower factory) — classified «documented environment-metadata», already treated as an accepted diff class in the pinned comparisons (see the 2.2.2 exit-gate and A5/A8 Evidence entries). Record only; no code change implied.
...
### 2026-08-04 — Product reframe: unified comparison model + placement detector family recorded (docs-only, branch `docs/product-reframe`, NO measurement in this session)

**Nature of this entry — read it before treating anything here as evidence.** This is a **docs-only** entry. Unlike every other entry in this log, it records **no measurement taken in this session**: no Unity was launched, no test suite was run, no C# was written or modified, no scene was opened. The findings behind the reframe come from a **research session on 2026-08-04** and were **not independently re-verified in this repository**. Full record with per-entry sources: **`docs/RESEARCH-2026-08-04-placement.md`** — that file's own header states the same standing, and every specific number in it must be re-verified here before any code depends on it.

**Decision recorded — the product reframe.** BugCam's measurement is one instrument, not several: **every measurement is a comparison of two states differing only in the reference point**, and there are four reference points — frame zero against itself (geometric findings: penetration, clearance, duplicates), a run against frame zero (settle findings), a run against another run (repeatability), a run against a perturbed run (fragility and threshold). The divergence engine is shared across all four; only the epsilon threshold search is specific to the fourth, because at epsilon zero there is no threshold to search. Written into `docs/SPEC.md` as new section 20 (additive; sections 1–19 keep their numbering). The placement detector family — static clash probe, settle probe, support-polygon stability margin, gap/leak detector, and a mandatory non-optional triage layer — is written into `docs/SPEC.md` section 21, **explicitly marked scope expansion and explicitly not part of v0.1**.

**Blocks 2.2.3 through 2.6 are UNAFFECTED and REMAIN THE CRITICAL PATH.** Not a single PLAN entry for 2.2.3, 2.3, 2.4, 2.5 or 2.6 was altered by this reframe, and no ratified contract was touched. The reason is structural rather than diplomatic: the evidence layer those blocks build is **required under every one of the four reference points** — a placement finding without a camera, a number and an evidence artifact is not a BugCam finding either. Phase 3 is appended to `docs/PLAN.md` **below** the Day 3 buffer (so `DAY 2 CHECKPOINT` stays attached to the Day 2 blocks it closes), and every Phase 3 block carries the status **`PLANNED-NOT-RATIFIED`** plus an explicit note that no code may begin until that block's contract is ratified — the 2.2.1 / 2.2.2 / 2.2.3 "contract first, no code" precedent applies unchanged.

**Two BLOCKING probes gate the Phase 3 contract (`docs/PLAN.md` Block 3.0).** Neither has been run; the Phase 3 contract cannot be written or ratified until both have, with numbers recorded here:
1. **Rigidbody census on `Assets/BugCam/Tests/GateScene222.unity`** — how many GameObjects carry a Rigidbody versus how many carry a Collider without one. That ratio decides whether the **static clash probe** or the **settle probe** is the primary detector: objects without a Rigidbody are never moved by simulation and therefore cannot be detected by it at all. **Read-only census — the scene is frozen by the Block 2.2.2 adjudication and must not be modified.**
2. **Sanitization cost of `Physics.ComputePenetration` on this project's Unity version** — how often the defect class reproduces on convex versus non-convex pairs, and what the multi-sample workaround actually costs in wall-clock time on a representative scene.

**Known issues recorded from the research (both now carried in Open findings / blockers above):** (a) raw `Physics.ComputePenetration` output is not trustworthy without sanitization — the recorded defect instance is an absurd **743.8444** depth for a shallow capsule-versus-non-convex-mesh overlap, or a `false` return at small depth, with a community workaround of 16-or-more calls at varied positions, implying the flagship static detector may be up to **16× more expensive than assumed**; (b) **PhysX determinism is a within-platform, within-version claim only** — divergence across CPU vendors (Intel versus AMD) or from GPU operation ordering is expected, not a defect. Constraint (b) is now stated directly in `docs/SPEC.md` §14, replacing the softer "environment-scoped" wording; constraint (a) is added there as a new caveat. `docs/SPEC.md` §15 gains two prohibitions: no AI in the triage layer, and no automated attempt to distinguish a bug from designer intent — the only permitted mechanism is a user-maintained plain-text exclusion list stored in the project.

**Open backlog item recorded (no code implied):** the existing **amplification** metric must be reconciled with the **finite-time Lyapunov exponent** definition and relabelled accordingly **at phase close** — see the OPEN entry in Open findings / blockers for the precise statement of the difference. Nothing was changed now: no number, no formula, and no `SPEC.md` §2 / §10 / §17 text.

**Environment:** docs-only commit on branch `docs/product-reframe`, branched from `main` (`2d75db3`). No Unity session, no batchmode run, no test suite executed — therefore **no pin can have drifted and none was exercised**. `Assets/**` untouched; `TowerScene.unity`, `DominoScene.unity` and `GateScene222.unity` untouched; the four protected files remain untracked. Note on the base: `docs/CONTRACT-2.2.3.md` is ratified on `feat/block-2.2.3` and did not exist on the main-based branch this entry records — it was neither read from nor written to by this commit, and nothing here depends on its state. **Updated 2026-08-04 after `main` was merged into `feat/block-2.2.3`: the contract file is now present alongside this entry; the statement above stays historically accurate for the reframe commit's own base and is not retracted.**

### 2026-08-04 — MediaEncoder probe: empirical H.264/MP4 verification for the Block 2.3 video-export adjudication (docs-only, `main`)

**Purpose:** the research pass recommending `UnityEditor.Media.MediaEncoder` over the Unity Recorder package left one premise unverified — no Unity documentation page affirmatively states that MediaEncoder writes H.264/MP4 on Windows without extra native dependencies. Settled empirically before ratification (the PLAN Block 2.3 decision now points here). Method: live Editor Unity `6000.3.21f1` (NOT batchmode — `run-checkpoint.ps1` runs `-nographics`, see the CONSTRAINT entry in Open findings); throwaway C# executed in-memory via mcp-for-unity `execute_code` — no script file ever existed under `Assets/`; the output `.mp4` was written to the session scratchpad outside the repo and deleted after measurement. Nothing committed from the probe itself; this entry is the surviving record.

**VERIFIED FACT — encoder run (execute_code result, verbatim):** `rate=50/1; colorSpace=Gamma; construct=OK; framesAdded=50/50; dispose=OK; file=EXISTS size=25130 bytes`. Constructor `new MediaEncoder(path, VideoTrackAttributes)` with width 1920, height 1080, `frameRate = MediaRational(50)` (recorded as the exact rational 50/1), `includeAlpha=false`; exactly 50 `AddFrame(Texture2D)` calls, all returned `true`; `Dispose()` clean; zero exceptions anywhere.

**VERIFIED FACT — file header (box-level parse of the produced file):** container MP4 — `ftyp` major brand `mp42`, compatible `mp41`, `isom`; single video track, sample entry `avc1` with an `avcC` configuration box reporting **H.264 Baseline profile, level 4.2**, 1920×1080. Independently confirmed by a decoder (OpenCV 4.11): fourcc `h264`, 50.0 fps, exactly 50 frames, first frame decodes.

**VERIFIED FACT — pixel checks on the decoded first frame:** no vertical flip — the texture-space top half (written red 200,40,40) decoded at the image top (pixel BGR `[41, 21, 188]`), the bottom half (written blue 40,40,200) at the image bottom (`[193, 28, 42]`). Mid-grey patch written at sRGB 128 read back at mean BGR `[129.5, 127.3, 130.8]` — within 2 of the written value, i.e. no linear/sRGB shift (deviation within YUV round-trip noise).

**VERIFIED FACT — dependency claim:** ffmpeg is absent from the machine (`where ffmpeg` — empty) and the Unity Recorder package was never installed; the built-in editor encoder did all of it.

**Limitations (ratified with the decision):** (a) the no-sRGB-shift result is proven for a **Gamma color space** project only (`colorSpace=Gamma` captured in the same run); a Linear project requires a separate measurement before the same claim is made. (b) MediaEncoder defaults to **H.264 Baseline profile level 4.2** — accepted as-is for v0.1; backlog only, no code implied: if output quality proves insufficient for the landing page or hero video, revisit with `VideoTrackEncoderAttributes` + `H264EncoderAttributes` at Main or High profile.

**Environment:** editor launched fresh for the probe and left running; no `ProjectSettings` drift (`git status` clean); `TowerScene.unity` untouched; the four protected files remain untracked; no test suites were run (no pins exercised, none can have drifted).
...
## Decisions log
- 2026-08-04 — Product reframe recorded as docs only: BugCam's measurement is **one comparison against four reference points** (`SPEC.md` §20 — frame zero vs itself, run vs frame zero, run vs run, run vs perturbed run; the divergence engine is shared, only the epsilon search is specific to the fourth), and the **placement detector family** is added as an explicit scope expansion outside v0.1 (`SPEC.md` §21, `PLAN.md` Phase 3, every block `PLANNED-NOT-RATIFIED`). Blocks 2.2.3–2.6 deliberately left untouched and kept as the critical path, because the evidence layer is required under every reference point — a placement finding without a camera, a number and an artifact is not a BugCam finding. Phase 3 is placed below the Day 3 buffer rather than literally between Block 2.6 and `DAY 2 CHECKPOINT`, so the Day 2 gate stays attached to the Day 2 blocks it closes. Research provenance and its explicit non-verification: `docs/RESEARCH-2026-08-04-placement.md`.
