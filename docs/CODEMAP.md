...
- **Выходной гейт 2.2.2 (сторонние меши):** `Tests/GateScene222.unity` + `Tests/GateScene222Assets/` (KayKit Dungeon Remastered 1.0, CC0, 8 .obj + лицензия; все меши Read/Write OFF) — 4 non-convex статики + 6 convex динамик (триггер body 1 = наклонённая колонна); run `ghost-20260804T074743614…`: захват Succeeded=True без fail-closed, hash `8b8e9a94…44b927`, meshRef на всех 10 объектах, вердикт **DIVERGENT AT SEARCH FLOOR — принят человеком как финальный** (сцена заморожена, не тюнинговать).
...
- **Mesh-захват (блок 2.2.2, реализован 2026-08-04; контракт `docs/CONTRACT-2.2.2.md`):** два расширения захвата (статический MeshCollider любой convexity → CapturedStatic; динамика + convex MeshCollider → CapturedDynamic). Меш по ссылке: `SimulationMeshReference` (guid + localFileId + contentHash геометрии всех субмешей + структурный отпечаток vertexCount/subMeshCount/bounds) через `Core/SceneMeshResolve.cs` (интерфейс + статический seam; реализация — `Editor/SceneMeshResolveProviderInstaller.cs`; нет провайдера = fail-closed). Поправка 2026-08-04: авторитетный захват — в edit mode ДО входа в Play Mode (`GhostEvidencePlayModeHost.StartTowerSearch` → `Editor/SceneCaptureHandover.cs`, JsonUtility-handover через domain reload, бит-точность float закреплена тестом); симуляционная точка (`SimulationHarness.ResolveMeshForSimulation`) геометрию не читает — резолв + бит-в-бит отпечаток, отказ = `SCENE_MESH_RESOLVE_FAILED`; ран сцены без handover = тот же код (инвариант П.6). Size меш-тел = мировой AABB (`mesh.bounds × |lossyScale|`), objectScale = max компонента. Хэш захвата аддитивен: Box/Sphere-строки байт-в-байт, меш-строки получают хвост `|mesh:guid:fileId:hash:convex`; manifest `objects[]` — поле `meshRef` только у меш-объектов. Пины: `Tests/EditMode/MeshCaptureTests.cs` (23), `Tests/PlayMode/MeshSimulationPlayModeTests.cs` (3).
