...
- Decided 2026-08-04 in the planning session as an explicit insert between 2.2.2 and 2.3; written into PLAN 2026-08-04 after the PR #11 merge. Contract ratified by the human 2026-08-04 (open questions №1–№11 adjudicated, including the literal-11 fork; empirical preconditions П-1…П-5 executed, all green). **Five amendments ratified by the human 2026-08-05** — handover `BodyDto` fields, static keys never exposed outward, a non-public structured launch entry point, separate compared sets for bodies and statics, and the row-12 scale channel confirmed by the pin re-run `Library/BugCamEvidence/probe-2.2.3-v1-pins/REPORT.md`; the superseded wording stays visible in place (contract section «Поправки 2026-08-05»). **Full ratified contract text: `docs/CONTRACT-2.2.3.md`** (this section is the summary; contradiction between the two = STOP and adjudicate).
